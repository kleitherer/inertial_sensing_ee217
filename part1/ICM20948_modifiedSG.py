...
def wrap_angle_180(angle):
    """
    Wrap angle to -180 to 180 degrees range.
    """
    while angle > 180.0:
        angle -= 360.0
    while angle < -180.0:
...
class ICM20948(object):
  def __init__(self,address=I2C_ADD_ICM20948):
    ...
    self._write_byte( REG_ADD_GYRO_CONFIG_1 , REG_VAL_BIT_GYRO_DLPCFG_6 | REG_VAL_BIT_GYRO_FS_500DPS | REG_VAL_BIT_GYRO_DLPF)
    ...
    # Store the configured ranges as instance variables (single source of truth)
    self._gyro_range_dps = 500  # Matches REG_VAL_BIT_GYRO_FS_500DPS
    self._accel_range_g = 2     # Matches REG_VAL_BIT_ACCEL_FS_2g
    
    #user bank 0 register
    self._write_byte( REG_ADD_REG_BANK_SEL , REG_VAL_REG_BANK_0) 
    time.sleep(0.1)
    self.icm20948GyroOffset()
    # self.icm20948MagCheck()  # Magnetometer disabled
    # self.icm20948WriteSecondary( I2C_ADD_ICM20948_AK09916|I2C_ADD_ICM20948_AK09916_WRITE,REG_ADD_MAG_CNTL2, REG_VAL_MAG_MODE_20HZ)  # Magnetometer disabled
  ...
  def imuAHRSupdate(self,gx, gy,gz,ax,ay,az,mx,my,mz):    
    ...
    # Magnetometer code commented out - not needed
    # norm = float(1/math.sqrt(mx * mx + my * my + mz * mz))      
    # mx = mx * norm
    # my = my * norm
    # mz = mz * norm

    # compute reference direction of flux (magnetometer disabled)
    ...
    # wx = 2 * bx * (0.5 - q2q2 - q3q3) + 2 * bz * (q1q3 - q0q2)  # Magnetometer disabled
    # wy = 2 * bx * (q1q2 - q0q3) + 2 * bz * (q0q1 + q2q3)  # Magnetometer disabled
    # wz = 2 * bx * (q0q2 + q1q3) + 2 * bz * (0.5 - q1q1 - q2q2)  # Magnetometer disabled

    # error is sum of cross product between reference direction of fields and direction measured by sensors
    # Using only accelerometer (magnetometer disabled)
    ex = (ay * vz - az * vy)  # + (my * wz - mz * wy)  # Magnetometer part disabled
    ey = (az * vx - ax * vz)  # + (mz * wx - mx * wz)  # Magnetometer part disabled
    ez = (ax * vy - ay * vx)  # + (mx * wy - my * wx)  # Magnetometer part disabled
    ...
    # Normalize quaternion (avoid division by zero)
    quat_magnitude = math.sqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3)
    if quat_magnitude > 0.0:
        norm = float(1.0 / quat_magnitude)
        q0 = q0 * norm
        q1 = q1 * norm
        q2 = q2 * norm
        q3 = q3 * norm
  ...
  def icm20948CalAvgValue(self):
    """
    From the datasheet: For 500 dps, (GYRO_FS_SEL=1), sensitivity is 65.5 LSB/dps.
    Therefore, we divide by 65.5 to get the gyro value in dps.
    """
    global MotionVal
    # Calculate conversion factors based on configured ranges
    gyro_lsb_per_dps = 65.5
    accel_denom = 32768.0 / self._accel_range_g # This becomes 16384 counts per g.

    MotionVal[0]=Gyro[0]/gyro_lsb_per_dps
    MotionVal[1]=Gyro[1]/gyro_lsb_per_dps
    MotionVal[2]=Gyro[2]/gyro_lsb_per_dps

    # Convert accelerometer from g to m/s² (1 g = 9.81 m/s²)
    MotionVal[3]=(Accel[0]/accel_denom) * 9.81
    MotionVal[4]=(Accel[1]/accel_denom) * 9.81
    MotionVal[5]=(Accel[2]/accel_denom) * 9.81
    # MotionVal[6]=Mag[0]  # Magnetometer disabled
    # MotionVal[7]=Mag[1]  # Magnetometer disabled
    # MotionVal[8]=Mag[2]  # Magnetometer disabled
...
if __name__ == '__main__':
  ...
  csv_filename = os.path.join(script_dir, 'rotation_around_x_0_to_180.csv') 
  # csv_filename = os.path.join(script_dir, 'rotation_around_y_0_to_180.csv')
  ...
  with open(csv_filename, 'w', newline='') as csvfile:
    ...
    csvwriter.writerow([
      'timestamp',
      'gyro_angle_around_y', 'gyro_angle_around_x',
      'accel_angle_around_y', 'accel_angle_around_x',
      'dt',
  ...
  print(f"Logging sensor data to: {csv_filename}\n")
  
  # Initialize variables for gyroscope integration
  gyro_angle_around_y = 0.0  # Cumulative angle from gyro Y integration (degrees)
  gyro_angle_around_x = 0.0
  gyro_z_angle = 0.0
  
  """
  Note: Previous way of using default dt caused issues - instead of 0.1, actual dt was 0.3 due to other delays.
  We set a target dt and set the sleep time accordingly to maintain 10 Hz sampling rate and accurate angle estimation.
  """
  TARGET_DT = 0.1
  next_t = time.time()
  prev_time = next_t
  ...
  try:
    while True:
      ...
      # icm20948.icm20948MagRead() # no need to read mag value; this slows it down
      icm20948.icm20948CalAvgValue()  # Uses ranges configured in __init__

      now = time.time()
      dt = now - prev_time
      prev_time = now

      # Integrate gyroscope angles using actual measured time step
      gyro_angle_around_y += MotionVal[1] * dt
      gyro_angle_around_x += MotionVal[0] * dt
      gyro_angle_around_y = wrap_angle_180(gyro_angle_around_y)
      gyro_angle_around_x = wrap_angle_180(gyro_angle_around_x)
      """
      Notes:
      * 0.0175 converts degrees to rad/s
      """
      icm20948.imuAHRSupdate(MotionVal[0] * 0.0175, MotionVal[1] * 0.0175,MotionVal[2] * 0.0175,
                  MotionVal[3],MotionVal[4],MotionVal[5], 
                  0.0, 0.0, 0.0)  # Magnetometer disabled - passing zeros

      pitch = wrap_angle_180(math.asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3)
      roll  = wrap_angle_180(math.atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3)
      yaw   = wrap_angle_180(math.atan2(-2 * q1 * q2 - 2 * q0 * q3, 2 * q2 * q2 + 2 * q3 * q3 - 1) * 57.3)
      accel_angle_around_y = wrap_angle_180(math.degrees(math.atan2(MotionVal[3],MotionVal[5])))
      accel_angle_around_x = wrap_angle_180(math.degrees(math.atan2(MotionVal[4],MotionVal[5])))
      print("\r\n /-------------------------------------------------------------/ \r\n")
      # print('\r\n Roll = %.2f , Pitch = %.2f , Yaw = %.2f\r\n'%(roll,pitch,yaw))
      print('\r\n Angle from Accelerometer around X (°)= %.2f \r\n'%(accel_angle_around_x))
      print('\r\n Angle from Accelerometer around Y (°)= %.2f \r\n'%(accel_angle_around_y))
      

      # print('\r\nAcceleration:  X = %d , Y = %d , Z = %d\r\n'%(Accel[0],Accel[1],Accel[2])) 
      print("Acceleration (m/s^2): X = %.3f, Y = %.3f, Z = %.3f" % (MotionVal[3], MotionVal[4], MotionVal[5]))

      # print('\r\nGyroscope:     X = %d , Y = %d , Z = %d\r\n'%(Gyro[0],Gyro[1],Gyro[2]))
      print('\r\nGyroscope (deg/s):     X = %.2f , Y = %.2f , Z = %.2f\r\n'%(MotionVal[0],MotionVal[1],MotionVal[2]))
      

      print("DEBUG: dt=%.4f, gyro2(dps)=%.2f, delta(deg)=%.2f"
      % (dt, MotionVal[2], MotionVal[2]*dt))

      gyro_y_angular_velocity = MotionVal[1]
      gyro_x_angular_velocity = MotionVal[2]
      print('\r\nGyroscope Y: %.2f DPS, dt: %.4f s' % (gyro_y_angular_velocity, dt))
      print('\r\nIntegrated Angle from Gyro around X: %.2f°\r\n' % gyro_angle_around_x)
      # Flip sign of gyro_angle_around_y to match accel direction
      gyro_angle_around_y_display = -gyro_angle_around_y
      print('\r\nIntegrated Angle from Gyro around Y: %.2f°\r\n' % gyro_angle_around_y_display)

      
      # Log to CSV: roll from accelerometer and integrated angle from gyroscope Y
      with open(csv_filename, 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        timestamp = datetime.now().isoformat()
        csvwriter.writerow([
          timestamp,
          wrap_angle_180(-gyro_angle_around_y),  # Flip sign to match accel direction
          wrap_angle_180(gyro_angle_around_x),
          wrap_angle_180(accel_angle_around_y),
          wrap_angle_180(accel_angle_around_x),
          dt,

          MotionVal[0], MotionVal[1], MotionVal[2],        # gyro in deg/s
          MotionVal[3], MotionVal[4], MotionVal[5],        # accel in m/s^2

          Gyro[0], Gyro[1], Gyro[2],                       # raw gyro counts (offset-corrected)
          Accel[0], Accel[1], Accel[2],                    # raw accel counts
        ])

      
      # Schedule next tick to maintain 10 Hz sampling rate
      next_t += TARGET_DT
