...
# IMPORTANT:  icm20948_Gyro_Accel_Read() currently SUBTRACTS GyroOffset.
# We will work around that by also capturing GyroOffset and reconstructing raw values.

def now_s():
    return time.time()

def main():
    out_csv = "tilt_axis2.csv"
    fs = 100.0            # logging rate (Hz) - pick 50 or 100
    duration_s = 4500   # 1 hour (set to 10.0 for quick test)
    flush_every = 500     # flush to disk periodically
    ...
    gx_off, gy_off, gz_off = GyroOffset[0], GyroOffset[1], GyroOffset[2]
    time.sleep(0.5)

    with open(out_csv, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["# fs_hz", fs])
        writer.writerow(["# duration_s", duration_s])
        writer.writerow(["# gyro_offset_counts_x", gx_off])
        writer.writerow(["# gyro_offset_counts_y", gy_off])
        writer.writerow(["# gyro_offset_counts_z", gz_off])
        writer.writerow([])
        writer.writerow([
            "t_s",
            "accel_x", "accel_y", "accel_z",
            "gyro_x_raw", "gyro_y_raw", "gyro_z_raw",
            "gyro_x_corr", "gyro_y_corr", "gyro_z_corr"
        ])
        ...
        for i in range(n_samples):
            ...
            gx_corr, gy_corr, gz_corr = Gyro[0], Gyro[1], Gyro[2]
            
            gx_raw = gx_corr + gx_off
            gy_raw = gy_corr + gy_off
            gz_raw = gz_corr + gz_off
